...
import { useSelector } from "react-redux"; // Nếu vẫn cần redux, có thể giữ
...
function Chat(props) {
  ...
  const handlerSend = async () => {
    // Kiểm tra nếu textMessage là "/end" thì kết thúc room
    ...
    // Kiểm tra nếu roomId không tồn tại thì tạo room mới
    if (!roomId) {
    ...
    // Gửi dữ liệu vào API để lưu trữ tin nhắn
    await ChatRoomsAPI.addMessage(data);
    setTextMessage("");

    setTimeout(() => {
      setLoad(true);
      socket.emit("send_message", data); // Emit socket
    }, 200);
  };
  ...
  // Sử dụng useEffect để gọi lại fetch dữ liệu khi có tin nhắn mới
  useEffect(() => {
    if (load) {
      fetchData();
      setLoad(false);
  ...
  // Nhận tin nhắn từ server qua socket
  useEffect(() => {
